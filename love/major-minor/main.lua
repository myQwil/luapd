...
function love.load()
	...
	local maj = { dest = 'maj-min', min = 1, max = 0, num = 1
		, snap = .25, gap = 12, change = sclChange }
	local scl = { dest = 'mode', min = 0, max = 7, num = 0
		, snap = .5, change = sclChange }
	local phase = { dest = 'phase', min = .5, max = 0, num = .5
		, snap = 1 / 48, gap = 0 }
	...
	buttons = {
		...
		, gui.button(bx + 75, 50, { label = { text = 'inv+' }
			, click = invup })
		, gui.button(bx + 300, 50, { label = { text = 'melodic-minor' }
			, click = melmin })
		, gui.button(bx + 300, 100, { label = { text = 'mixo-b6', y = 60 }
			, click = mixob6 })
		, gui.button(bx + 375, 75, { label = { text = 'stop', x = 40, y = 30 }
			, click = stop })
	}
